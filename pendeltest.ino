...
void loop() {
  ...
  pendelAngelCalk();
  if (pendelPosDeg > 175 && pendelPosDeg < 185){
    x = (millis()*0.001 - 10)*360;
    if (x<0){
      x = 0;
    }
    errPendel = (180 - pendelPosDeg)*(-30);
    errPendelDPS = (0-pendelDPS)*(-0.5);
    errVelocity = (360-motorVelocety)*(-0.8);
    errMotorPos = (x-posDeg)*(-0.5);
    totErrPendel = errPendel + errPendelDPS+errVelocity+errMotorPos;
    pendelMotor_.runMotorPWM(totErrPendel, posDeg, setPosMotor);    
  }
  else if (pendelPosDeg > 155 && pendelPosDeg < 205){
    ...
    if (pendelDPS<0){
      pendelMotor_.getUp(60, 1);
    }
    else{
      pendelMotor_.getUp(200, 0);
    }
  }
  else if ((pendelPosDeg > 370 && pendelPosDeg < 420)|| (pendelPosDeg < 350 && pendelPosDeg > 300)){
    if (pendelDPS<0){
      pendelMotor_.getUp(200, 1);
    }
    else{
      pendelMotor_.getUp(60, 0);
    }
  }
  else{
